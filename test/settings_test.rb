...
class SettingsTest < Test::Unit::TestCase
  ...
  def test_with_a_single_file
    Settings.setup { file("test/config1.yml") }
    assert_equal("dev.something.com", Settings.host)
    assert_equal(%w[www.google.com www.yahoo.com], Settings.urls)
    assert_equal(11, Settings.num_count)
  end

  def test_with_nesting_in_file
    Settings.setup { file("test/config2.yml") }
    assert_equal("prod.something.com", Settings.host)
    assert_equal "blah", Settings['nest.something']
    assert_equal "coco", Settings['nest.name.fname']
    assert_equal({ 'nest.name.fname' => 'coco', 'nest.name.lname' => 'bottaro' },
      Settings.all('nest.name'))
    assert_equal "here", Settings['nest.back']
    assert_equal "end", Settings.finally
  end

  def test_with_two_files
    Settings.setup do
      file("test/config1.yml")
      file("test/config2.yml")
    end
    assert_equal(11, Settings.num_count)
    assert_equal("prod.something.com", Settings.host)
    assert_equal "blah", Settings['nest.something']
